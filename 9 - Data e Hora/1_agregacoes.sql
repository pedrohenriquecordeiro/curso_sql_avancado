...
SELECT B.regiao,MAX(a.POPULACAO)maximo 
FROM senso a
  INNER JOIN regiao B
	ON A.cod_uf=B.cod_uf
GROUP BY B.regiao
...
from senso
group by uf with rollup -- retorna o resultado total do nao agrupamento
...
(select cod_uf,MAX(populacao) as populacao from senso group by cod_uf) b
	join senso a
		on a.cod_uf=b.cod_uf and a.populacao=b.populacao
...
HAVING populacao = (SELECT MAX(populacao) FROM senso b WHERE b.cod_uf = a.cod_uf)
ORDER BY a.populacao desc
