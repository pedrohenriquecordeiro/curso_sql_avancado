...
--LIKE Localiza valores que nao começam com "a"	
	SELECT  a.primeiro_nome,
	       a.ultimo_nome  
	FROM   clientea
...
--OPERADOR EXISTS -- semelhante ao IN
...
--MESMO EXEMPLO UTILIZANDO ANY -- semelhante ao EXISTS
...
use CURSO
--OPERADOR HAVING -- filtro no agregador
SELECT  a.uf,
		Count(a.cod_mun) as qtd_cidades
FROM   SENSO a
GROUP  BY  a.uf HAVING  Count(a.cod_mun) > 100 -- FILTRO  
order by 2 desc; -- ordena pela 2a coluna
